function NLL = get_gaussian_NLL(X, mu_resp, conf_resp, params, nSamples)

for i_trial = 1:length(X)
    S = X{i_trial};
    for i_sample = 1:nSamples
        [mu_hat_samples(i_sample), conf_hat_samples(i_sample), iter(i_sample), mu_hat_all{i_sample}] = func_iter_avg_gaussian_maxiter(params, S, nSamples);
    end
