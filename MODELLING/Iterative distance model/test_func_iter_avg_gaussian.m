...
beta = log(7);
lapse = log(0.1);
params = [alpha, k_sig_scale, convergence_threshold, beta, lapse];
...
for i_trial = 1:length(X)
    x = X{i_trial};
    [model.mu_hat(i_trial), model.conf_hat(i_trial), nIter(i_trial), mu_hat_all{i_trial}] = func_iter_avg_gaussianapprox(params,x);%func_iter_avg_single(params,x);%func_iter_avg(params, x,nMeasurements);
