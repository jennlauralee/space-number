...
for i_trial = 1:length(X)
    x = X{i_trial};
    [model.mu_hat(i_trial), model.conf_hat(i_trial), iter(i_trial)] = func_iter_avg_lognormal_single(params,x);%func_iter_avg_single(params,x);%func_iter_avg(params, x,nMeasurements);
