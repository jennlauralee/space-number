...
%% Load parameters
params = [log(1), log(0.1), log(3),log(8),log(5)];
...
for i_trial = testtrials
    ...
    for i_sample = 1:nSamples
        [mu_hat_samples(i_sample), conf_hat_samples(i_sample), iter(i_sample), mu_hat_all{i_sample}] = func_iter_avg_gaussianapprox(params, S);
    end
